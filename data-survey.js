...
data_survey_form.find(`[type="submit"]`).hide()
jQuery.get(data_survey.url, function (response) {
    if (response) {
        ...
        data_survey_form.find(`[name="item_meta[3890]"]`).val(data_survey_chosen_field).trigger('change')
        data_survey_form.find(`[name="item_meta[3891]"]`).val(data_survey_chosen_display_type)
